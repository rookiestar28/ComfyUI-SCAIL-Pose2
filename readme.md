...
| `SCAILPose2ReferenceImageGeometryAlign` | `ref_image`: reference subject image.<br>`ref_mask`: SCAIL-2 reference semantic mask.<br>`pose_video_mask`: driving semantic mask sequence used as the target canvas and bbox source.<br>`fit_mode`: `contain`, `cover`, `fit_height`, or `fit_width`.<br>`anchor`: `bottom_center` or `center`.<br>`target_frame_policy`: `median_bbox`, `first_valid`, or `largest`.<br>`bbox_margin`, `max_scale`, `min_mask_area_ratio`: bounded placement and validation controls. | `ref_image` (`IMAGE`), `ref_mask` (`IMAGE`), `summary` (`STRING`) | Aligns a mismatched replacement reference image/mask to the driving mask canvas before `SCAILPose2SCAIL2Condition`. It infers target size from `pose_video_mask`; there are no width/height widgets. Use this when the reference crop, aspect ratio, or subject scale differs from the original driving subject. |
...
When the reference image has a different crop, aspect ratio, or subject scale from the driving subject, place `SCAILPose2ReferenceImageGeometryAlign` before `SCAILPose2SCAIL2Condition`: connect `ref_image` and `ref_mask` into the align node, connect `SCAILPose2ColoredMask.pose_video_mask` as its target mask, then feed the aligned `ref_image` and `ref_mask` into Condition. The default `contain` + `bottom_center` + `median_bbox` policy is intended for full-body replacement references and avoids direct reference-image stretching.
